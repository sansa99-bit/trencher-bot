...
interface CacheEntry { ts: number; data: Holding[] }
const cache = new Map<string, CacheEntry>();
const CACHE_TTL_MS = 60_000;

async function rpc(method: string, params: unknown[]): Promise<any> {
  const { helius } = loadConfig();
  if (!helius.rpcUrl) throw new Error('RPC indisponible');
...
async function mintCreatedAt(mint: string): Promise<number | undefined> {
  try {
...
export async function walletHoldings(wallet: string): Promise<Holding[]> {
