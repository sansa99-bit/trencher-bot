...
export type DecodedEvent = DecodedTrade | DecodedCreate;
...
function decodeTrade(buf: Buffer): DecodedTrade | null {
  if (buf.length < TRADE_MIN_LEN) return null;
  let o = 8;
  const mint = toBase58(buf.subarray(o, o + 32)); o += 32;
  const solAmount = Number(buf.readBigUInt64LE(o)); o += 8;
  const tokenAmount = Number(buf.readBigUInt64LE(o)); o += 8;
  const buyByte = buf.readUInt8(o); o += 1;
  if (buyByte > 1) return null;
  const wallet = toBase58(buf.subarray(o, o + 32)); o += 32;
  const ts = Number(buf.readBigInt64LE(o)); o += 8;
  const virtualSolReserves = Number(buf.readBigUInt64LE(o)); o += 8;
  const virtualTokenReserves = Number(buf.readBigUInt64LE(o));
  if (virtualTokenReserves <= 0 || virtualSolReserves <= 0) return null;
  if (ts < 1_600_000_000 || ts > 4_000_000_000) return null;
...
function readStr(buf: Buffer, o: number): { value: string; next: number } | null {
  if (o + 4 > buf.length) return null;
  const len = buf.readUInt32LE(o);
  if (len > 200 || o + 4 + len > buf.length) return null;
  return { value: buf.subarray(o + 4, o + 4 + len).toString('utf8'), next: o + 4 + len };
}

function decodeCreate(buf: Buffer): DecodedCreate | null {
  let o = 8;
  const name = readStr(buf, o); if (!name) return null; o = name.next;
  const symbol = readStr(buf, o); if (!symbol) return null; o = symbol.next;
  const uri = readStr(buf, o); if (!uri) return null; o = uri.next;
  if (o + 96 > buf.length) return null;
  const mint = toBase58(buf.subarray(o, o + 32)); o += 32;
  const bondingCurve = toBase58(buf.subarray(o, o + 32)); o += 32;
  const creator = toBase58(buf.subarray(o, o + 32));
  if (!symbol.value || !mint) return null;
  return { type: 'create', mint, name: name.value, symbol: symbol.value, uri: uri.value, creator, bondingCurve };
}

export function decodeProgramData(base64: string): DecodedEvent | null {
  try {
