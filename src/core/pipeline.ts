...
import type { NarrativeState, TokenCreation, TokenState, Trade } from './types.js';
...
export function startPipeline(): () => void {
  ...
  const purgeTimer = setInterval(() => {
    ...
    if (removed > 0) log.debug(`Purge: ${removed} token(s) inactif(s) retiré(s).`);
  }, 60_000);
  purgeTimer.unref();
...
function handleCreation(creation: TokenCreation): void {
...
function handleTrade(trade: Trade): void {
  ...
  if (now - last < RESCORE_MIN_INTERVAL_MS) return;
...
function updateCurveAndMarketCap(state: TokenState, trade: Trade, buffer: ReturnType<typeof tokenStore.buffer>): void {
  if (trade.virtualSolReserves === undefined || trade.virtualTokenReserves === undefined) return;
  ...
  if (state.firstMarketCap === undefined) state.firstMarketCap = marketCap;
...
function markMigrated(state: TokenState): void {
...
function refreshMetrics(state: TokenState, buffer: NonNullable<ReturnType<typeof tokenStore.buffer>>, now: number): void {
...
function deriveNarrative(state: TokenState): NarrativeState {
  const accel = state.volume.volumeAcceleration ?? 0;
  const buyersDelta = state.uniqueBuyersDelta ?? 0;
  if (accel >= 1.8 && buyersDelta >= 0.15) return 'ACCELERATING';
  if (accel >= 1.2 || buyersDelta >= 0.05) return 'BUILDING';
  return 'FLAT';
}

function evaluate(state: TokenState): void {
  const breakdown = scoreToken(state);
