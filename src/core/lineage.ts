...
interface Lineage { count: number; bestMcap: number; lastAt: number }

interface Store {
  tickers: Record<string, Lineage>;
  creators: Record<string, Lineage>;
}

let state: Store = { tickers: {}, creators: {} };
let dirty = false;
...
function load(): void {
  ...
  try {
    ...
    if (parsed?.tickers) state = { tickers: parsed.tickers, creators: parsed.creators ?? {} };
...
function bump(map: Record<string, Lineage>, k: string): void {
  if (!k) return;
  const cur = map[k];
  if (cur) {
    cur.count++;
...
export function tickerLaunches(symbol: string): number {
  return state.tickers[key(symbol)]?.count ?? 0;
}

export function creatorLaunches(creator: string): number {
 ...
 */
export function isFarmClone(t: TokenState): boolean {
  ...
  if (!tickerFarm && !creatorFarm) return false;
  ...
  if (mcap >= HARD_PASS_MCAP || vol5m >= HARD_PASS_VOLUME) return false;
...
export function startLineageTracker(): () => void {
  load();

  const offCreated = bus.on('token:created', (c) => {
    bump(state.tickers, key(c.symbol));
    if (c.creator) bump(state.creators, c.creator);
  });
  ...
  const timer = setInterval(() => {
    for (const t of tokenStore.all()) {
      ...
      if (line && mcap > line.bestMcap) { line.bestMcap = mcap; dirty = true; }
      const cr = t.creator ? state.creators[t.creator] : undefined;
      if (cr && mcap > cr.bestMcap) { cr.bestMcap = mcap; dirty = true; }
    }
    save();
  }, 20_000);
  timer.unref();
