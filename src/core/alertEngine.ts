...
class AlertEngine {
  evaluate(input: EvaluateInput): boolean {
    ...
    if (score < config.alerts.minScore) return false;
    const level = levelForScore(score);
    if (level === 'WATCHLIST') return false;
  ...
  private decide(
    ...
    if (token.ignoredUntil && now < token.ignoredUntil) return null;
    if (RANK[ctx.level] > RANK[prev]) return { reason: 'niveau monté' };
    if (ctx.level !== prev && token.lastAlertKind !== 'EARLY') return { reason: 'changement de kind' };
    ...
    if (!cooldownOk || !scoreDeltaOk) return null;
