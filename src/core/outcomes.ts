...
export type Outcome = 'rug' | 'win' | 'neutral';
...
let state: Store = { scanned: 0, called: 0, outcomes: {} };

function load(): void {
  if (!existsSync(FILE)) return;
  try {
    const parsed = JSON.parse(readFileSync(FILE, 'utf8')) as Store;
    if (parsed && typeof parsed.scanned === 'number') state = parsed;
...
function classify(t: TokenState): Outcome | null {
  const mcap = t.marketCap ?? 0;
  const ath = t.athMarketCap ?? 0;
  const first = t.firstMarketCap ?? 0;

  if (ath >= RUG_PEAK_REQUIRED && mcap > 0 && mcap < RUG_COLLAPSE_MCAP) return 'rug';
  if (ath > 0 && mcap > 0 && 1 - mcap / ath >= RUG_DRAWDOWN) return 'rug';
  if (first > 0 && ath / first >= WIN_MULTIPLE) return 'win';
  return null;
}
...
export function startOutcomeTracker(): () => void {
  ...
  const scan = setInterval(() => {
    for (const t of tokenStore.all()) {
      ...
      if (current === 'rug') continue; // un rug ne se rachète pas
...
export function outcomeStats(): { scanned: number; called: number; rug: number; win: number; neutral: number } {
  let rug = 0;
  let win = 0;
