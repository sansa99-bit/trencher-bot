 ...
 */
import { LEVEL_THRESHOLDS, POST_WEIGHTS, PRE_WEIGHTS, REENTRY_WEIGHTS, } from '../config/scoring.js';
function ramp(value, lo, hi, max) {
    if (value === undefined || !Number.isFinite(value) || hi === lo)
        return 0;
...
function rampInverse(value, good, bad, max) {
    if (value === undefined || !Number.isFinite(value) || bad === good)
        return 0;
 ...
 */
function volumeScore(volume5m, marketCap, max) {
    if (volume5m === undefined || volume5m <= 0)
        return 0;
...
export function scorePreMigration(t) {
    const v = t.volume;
    return build([
...
export function scorePostMigration(t) {
    const v = t.volume;
    return build([
...
export function drawdown(t) {
    if (!t.athMarketCap || !t.marketCap || t.athMarketCap <= 0)
        return undefined;
 ...
 */
export function isReentryCandidate(t) {
    if (t.phase !== 'MIGRATED')
        return false;
    const dd = drawdown(t);
    if (dd === undefined)
...
export function scoreReentry(t) {
    const v = t.volume;
    const dd = drawdown(t);
    // Optimum autour de -45 % ; on s'éloigne, on perd des points.
    const ddValue = dd === undefined ? 0 : ramp(1 - Math.abs(Math.abs(dd) - 0.45) / 0.25, 0, 1, REENTRY_WEIGHTS.drawdown);
    return build([
...
export function levelForScore(score) {
    if (score >= LEVEL_THRESHOLDS.A_PLUS)
        return 'A_PLUS';
    if (score >= LEVEL_THRESHOLDS.A)
        return 'A';
    if (score >= LEVEL_THRESHOLDS.SIGNAL)
        return 'SIGNAL';
    if (score >= LEVEL_THRESHOLDS.WATCHLIST)
        return 'WATCHLIST';
    return 'NONE';
}
