...
export class MockScanner {
    ...
    spawn() {
        if (this.tokens.length > 12)
            this.tokens.shift();
        const profile = pick(['rug', 'slow', 'runner', 'survivor']);
    ...
    decideBuy(token) {
        if (token.profile === 'rug')
            return Math.random() < (token.ticks < 120 ? 0.8 : 0.12);
        if (this.inBurst(token))
            return Math.random() < 0.85;
        if (token.cycle === 'down')
            return Math.random() < 0.35;
        if (token.cycle === 'recovery')
            return Math.random() < 0.78;
        if (token.profile === 'runner')
            return Math.random() < 0.76;
        if (token.profile === 'survivor')
            return Math.random() < 0.68;
        return Math.random() < 0.5;
    }
    tradeSize(token, isBuy) {
        const base = isBuy ? randInt(0.1, 5) : randInt(0.05, 3);
        if (token.profile === 'rug' && !isBuy)
            return base * 2;
        if (this.inBurst(token))
            return base * 1.5;
        return base;
