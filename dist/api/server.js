...
function calledTokens(limit = 50) {
    return tokenStore
        .all()
...
function readWallets() {
    if (!existsSync(WALLETS_PATH))
        return [];
    try {
        const parsed = JSON.parse(readFileSync(WALLETS_PATH, 'utf8'));
    ...
    app.get('/api/diag', (_req, res) => {
        ...
        for (const t of all) {
            ...
            if ((t.volume.tradeCount1m ?? 0) < MIN_TRADES_1M) {
                fail.trades++;
                if (samples.length < 8)
                    samples.push({ sym: t.symbol, mcap, tc1m: t.volume.tradeCount1m ?? 0, hold: t.holders ?? 0, v5m: t.volume.volume5m ?? 0 });
                continue;
            }
            ...
            if ((t.volume.volume5m ?? 0) / mcap < MIN_TURNOVER_5M) {
                fail.turnover++;
                continue;
            }
            fail.alive++;
