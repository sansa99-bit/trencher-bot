...
export function isPlausibleMint(mint) {
...
export function parseCallback(data) {
    ...
    if (!mint)
        return null;
    if (!Object.values(CALLBACK).includes(action))
        return null;
    return { action: action, mint };
}
export function tokenKeyboard(token) {
    ...
    if (token.watched)
        kb.text('✅ WATCHED', `${CALLBACK.unwatch}:${mint}`);
    else
        kb.text('⭐ WATCH', `${CALLBACK.watch}:${mint}`);
