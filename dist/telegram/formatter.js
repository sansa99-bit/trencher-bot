...
export function formatPostMigration(event) {
    const t = event.token;
    const dd = drawdown(t);
    const facts = lines([
...
export function formatReentry(event) {
    const t = event.token;
    const dd = drawdown(t);
    const facts = lines([
        ['ATH', usd(t.athMarketCap)],
...
export function formatTokenLine(token, rank) {
...
export function formatProfitUpdate(update) {
